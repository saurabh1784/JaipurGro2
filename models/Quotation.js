...
async function listForClient(clientId) {
  ...
  const [rows] = await pool.query(
    `SELECT qr.id,
            ...
            COALESCE(NULLIF(NULLIF(vprod.image_url, ''), '/default.png'), NULLIF(NULLIF(p.image_url, ''), '/default.png'), '/default.png') AS image_url,
            NULLIF(NULLIF(vprod.image_url, ''), '/default.png') AS vendor_image_url,
            NULLIF(NULLIF(p.image_url, ''), '/default.png') AS product_image_url,
            GREATEST(
              EXTRACT(EPOCH FROM COALESCE(vprod.updated_at, p.updated_at)),
              EXTRACT(EPOCH FROM COALESCE(p.updated_at, vprod.updated_at))
