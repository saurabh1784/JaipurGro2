...
async function getBankAccount(req, res) {
  try {
    ...
    const bankAccount = await VendorWithdrawal.getBankAccount(currentActor.id);
...
async function saveBankAccount(req, res) {
  try {
    ...
    const bankAccount = await VendorWithdrawal.saveBankAccount(currentActor.id, {
      accountNumber,
      ifscCode,
      bankName,
      accountHolderName,
...
async function createRequest(req, res) {
  ...
  try {
    ...
    if (!bankDetails || !bankDetails.account_number) {
      const savedAccount = await VendorWithdrawal.getBankAccount(currentActor.id);
    ...
    const request = await VendorWithdrawal.createRequest(currentActor.id, {
      amount,
      bankDetails,
      note,
    });
    ...
    try {
      adminNotificationService.notifyAdmin({
        ...
        message: `${request.vendor_name} (${request.city}) submitted a withdrawal request of ₹${amount.toFixed(2)}.`,
    ...
    res.status(201).json({
      success: true,
      message: 'Withdrawal request submitted successfully! Pending approval from City Admin.',
      request,
    });
  } catch (error) {
...
async function listVendorWithdrawals(req, res) {
  try {
    const currentActor = actor(req);
    if (!currentActor) return res.status(401).json({ success: false, message: 'Authentication required' });

    const result = await VendorWithdrawal.listRequests({
      vendorId: currentActor.id,
      page: req.query.page,
    ...
    console.error('Error listing vendor withdrawals:', error);
...
async function listAdminWithdrawals(req, res) {
  try {
    ...
    if (req.query.format !== 'json' && req.accepts(['html', 'json']) !== 'json') {
      return res.render('vendor-withdrawals', {
    ...
    console.error('Error listing admin withdrawals:', error);
    res.status(500).json({ success: false, message: 'Unable to load withdrawal requests' });
  }
}
...
async function approveWithdrawal(req, res) {
  ...
  try {
    const request = await VendorWithdrawal.findById(id);
    ...
    try {
      ...
      const walletData = await Wallet.lockForUser(request.vendor_id, connection);
      if (walletData.balance < request.amount) {
        await connection.rollback();
        return res.status(422).json({
          success: false,
          message: `Vendor has insufficient wallet balance (Current balance: ₹${walletData.balance.toFixed(2)}).`,
        });
      }
      ...
      await connection.query(
        ...
        [request.amount, request.vendor_id, request.amount]
      );

      const [newWallet] = await connection.query('SELECT balance FROM wallets WHERE user_id = ? LIMIT 1', [request.vendor_id]);
      ...
      await connection.query(
        ...
        [
          ...
          `Vendor Withdrawal Approved: ${adminRemark || 'Processed by admin'}`,
          currentActor.id,
        ]
      );

      const updatedRequest = await VendorWithdrawal.approveRequest(id, {
        adminRemark,
      ...
      const [vendorRows] = await pool.query('SELECT name, email, phone FROM users WHERE id = ? LIMIT 1', [request.vendor_id]);
      const vendorUser = vendorRows[0] || {};

      try {
        notifyUserEvent({
          phone: vendorUser.phone || '',
          email: vendorUser.email || '',
          name: vendorUser.name || request.vendor_name || 'Vendor',
      ...
      res.json({
        success: true,
        message: `Withdrawal request of ₹${request.amount.toFixed(2)} approved & ₹${request.amount.toFixed(2)} deducted from vendor wallet.`,
...
async function rejectWithdrawal(req, res) {
  ...
  try {
    ...
    const updatedRequest = await VendorWithdrawal.rejectRequest(id, {
      adminRemark,
      processedByUserId: currentActor.id,
      processedByName: currentActor.name || 'Admin',
    });
