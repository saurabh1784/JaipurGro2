...
async function start({phone,appType,profile}) {
  ...
  const existing=await User.findByEmailOrPhoneIdentifierForApp(phone,normalized.appType);
...
async function promote(phone,appType,connection=pool) {
  ...
  const userId=await User.create({name:String(p.name).trim(),email:String(p.email).trim(),phone,password,role:registration.role,status:partner?'inactive':'active',city:p.city||null,area:p.area||null},connection);
  ...
  await pool.query("UPDATE otp_registrations SET mobile_verified=1,approval_status=?,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?",[partner?'pending_documents':'approved',partner?0:1,registration.id]);
