...
async function initializeSchema(pool) {
  ...
  await createIndex(pool, 'idx_wallets_status', 'wallets', 'status');
  await createIndex(pool, 'idx_products_name', 'products', 'name');
  await createIndex(pool, 'idx_products_deleted', 'products', 'is_deleted');
  await createIndex(pool, 'idx_vendor_products_vendor', 'vendor_products', 'vendor_id');
  await createIndex(pool, 'idx_client_orders_user', 'client_orders', 'user_id');
  await createIndex(pool, 'idx_client_orders_vendor', 'client_orders', 'vendor_id');
  await createIndex(pool, 'idx_quotation_vendor', 'quotation_vendor_recipients', 'vendor_id, status');
  await createUniqueIndex(pool, 'uniq_users_email', 'users', 'email');
  await createUniqueIndex(pool, 'uniq_users_phone', 'users', 'phone');

  await ensureColumn(pool, 'vendor_profiles', 'aadhaar_front_path', 'VARCHAR(255)');
  await ensureColumn(pool, 'vendor_profiles', 'aadhaar_back_path', 'VARCHAR(255)');
  await ensureColumn(pool, 'vendor_profiles', 'store_image_path', 'VARCHAR(255)');
  await ensureColumn(pool, 'vendor_profiles', 'profile_image_path', 'VARCHAR(255)');
}
