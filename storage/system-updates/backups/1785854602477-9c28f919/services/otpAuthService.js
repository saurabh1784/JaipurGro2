...
async function completeVerifiedPhoneLogin(phone, normApp) {
  ...
  if (!existingUser) {
    ...
    if (!existingUser && rawDigits.length > 10) {
      existingUser = await User.findByEmailOrPhoneIdentifier(rawDigits.slice(-10));
    }
  }
  ...
  if (existingUser) {
    ...
    if (['inactive', 'suspended', 'blocked', 'deleted'].includes(existingUser.status) || existingUser.is_deleted === 1) {
      const error = new Error(`Your account status is ${existingUser.status || 'inactive'}. Please contact your administrator.`);
      error.status = 403;
      throw error;
    }
    if ((normApp === 'vendor' || normApp === 'delivery') && existingUser.status !== 'active') {
  ...
  if (promoted) {
    if (normApp === 'vendor' || normApp === 'delivery') {
      const error = new Error('Your mobile number has been verified. Your profile is waiting for approval.');
      error.status = 403;
...
async function verifyOtp({ phoneInput, countryCodeInput, otp, appType = 'client', referralCode }) {
  ...
  if (existingUser) {
    ...
    if (existingUser.status === 'inactive' || existingUser.status === 'suspended' || existingUser.status === 'blocked' || existingUser.status === 'deleted' || existingUser.is_deleted === 1) {
      const message=existingUser.status==='inactive'?'Your account is currently inactive.':existingUser.status==='rejected'?'Your profile has been rejected. Please contact support.':`Your account status is ${existingUser.status || 'inactive'}. Please contact your administrator.`;
  ...
  if(promoted){
    if(normApp==='vendor'||normApp==='delivery'){
      const error=new Error('Your mobile number has been verified. Your profile is waiting for approval.');error.status=403;error.isNewPending=true;throw error;
    }
    return promoted;
  }
