...
async function getEmailSettings() {
  ...
  const fromName = await getSettingValue('email_from_name', 'GROXEN');
 ...
 */
async function sendTestEmail({ recipientEmail, subject, customBody, templateKey, config }) {
  ...
  if (templateKey && templateKey !== 'default_test') {
    ...
    if (rendered && rendered.isEnabled !== false) {
      mailSubject = mailSubject || rendered.subject;
      htmlContent = rendered.content;
      bodyText = bodyText || rendered.content.replace(/<[^>]*>?/gm, '');
    }
  }

  if (!mailSubject) mailSubject = 'GROXEN - Test Email Verification';
  if (!bodyText) bodyText = 'This is a test email sent from GROXEN Server to verify SMTP settings and email deliverability.';

  if (!htmlContent) {
    htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
      <div style="background: linear-gradient(135deg, #0284c7 0%, #0369a1 100%); padding: 20px; border-radius: 8px 8px 0 0; text-align: center; color: #ffffff;">
      ...
      <div style="padding: 24px; color: #334155; line-height: 1.6;">
        ...
        <div style="background-color: #f8fafc; border-left: 4px solid #0284c7; padding: 14px; margin: 20px 0; border-radius: 0 6px 6px 0;">
