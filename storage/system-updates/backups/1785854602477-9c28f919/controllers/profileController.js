...
async function updateProfile(req, res) {
  try {
    ...
    if (userRoleNorm === 'vendor' && (
      Object.prototype.hasOwnProperty.call(rawProfile, 'gst_number') ||
      Object.prototype.hasOwnProperty.call(rawProfile, 'gst_not_applicable')
    )) {
      const gstNotApplicable = rawProfile.gst_not_applicable === true ||
        String(rawProfile.gst_not_applicable || '').toLowerCase() === 'true';
      const gstNumber = String(rawProfile.gst_number || '').trim().toUpperCase();
      const gstPattern = /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/;

      if (!gstNotApplicable && !gstNumber) {
        return res.status(422).json({ success: false, message: 'Enter a valid GST Number or select GST Not Applicable.' });
      }
      if (!gstNotApplicable && !gstPattern.test(gstNumber)) {
        return res.status(422).json({ success: false, message: 'Enter a valid 15-character GST Number.' });
      }
      profileUpdate.gst_number = gstNotApplicable ? 'GST_NOT_APPLICABLE' : gstNumber;
    }
